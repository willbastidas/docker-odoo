...
export class AppsMenu extends Component {
    ...
    async validar() {
        const response = await this.orm.call(
            "fiscal.audit",
            "action_validar_ordenes_de_entrega",
            [1],
            {}
        );

        this.state.pending_delivery = !response;

    }
    
    async checkDeliveryOrders (){
        const domain = [
            ["picking_type_id.code", "=", "outgoing"],
            ["state", "not in", ["done", "cancel"]],
            ["scheduled_date", "<", new Date().toISOString()],
        ];
        const fields = ["name", "scheduled_date", "state"];
        try {
            const deliveryOrders = await this.orm.searchRead("stock.picking", domain, fields);
            if (deliveryOrders.length) {
                this.state.pending_delivery = true;
                console.log("Overdue delivery orders:", deliveryOrders);
            } else {
                this.state.pending_delivery = false;
                console.log("No overdue delivery orders found.");
            }
        } catch (error) {
